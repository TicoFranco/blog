...
export default function Login() {
  ...
  const {register,setValue,handleSubmit,formState: { errors }} = useForm<FormDataLogin>()
  ...
  return (
    <div className='bg-blue-950 min-h-screen'>
        ...
        <div className="flex flex-col justify-center items-center text-white mt-8">
            ...
            <p className=''>Don't have an account yet? <Link className='text-green-500' href={'/sign'}>Sign up</Link></p>
        </div>

        {state ? <Modal result={result} setResult={setResult} state={state} setState={setState} sucessText='Login successfully' failureText='Your email or password is incorrect.'/>: null}
    </div>
  )
}
