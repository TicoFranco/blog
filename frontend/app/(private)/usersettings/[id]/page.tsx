...
import { getCommentsUser } from "@/app/services/Comments/getCommentsUser"
import { getUserPosts } from "@/app/services/Posts/getUserPosts"

export default function page() {
  const router = useRouter()
  const {user,setUser} = useUserContextApi()
  const [updatedUser,setUpdatedUser] = useState<Iuser | null>(user)
  ...
  async function getData(formData:formDataUserSettings){
    if(!updatedUser){
      return 
    }

    if(attribute === "avatar"){
      setUpdatedUser({...updatedUser,avatarUrl:formData.avatar})
    }else if(attribute === "name"){
      setUpdatedUser({...updatedUser,name:formData.name})
    }else if(attribute === "password"){
      setNewPassword(formData.password)
    }else if(attribute === "Confirm changes"){
      const updateData:Iuser = {...updatedUser,password:formData.oldPassword}
      const res = await updateUser(updateData,newPassword)
      if("success" in res){
        setUser(updatedUser)
        setResult("success")
      }else{
  ...
  async function findData(){
    await getUserPosts().then(setPosts)
    await getCommentsUser().then(setComments)
  }
  ...
  return (
    <div className='bg-blue-950 min-h-screen'>
        ...
        <div className="flex flex-col mt-3 ml-5 mr-5">
            ...
            <h1 className="text-white text-5xl my-3 text-center">Settings</h1>
            <div className="bg-white rounded-md w-full max-w-4xl mx-auto mt-3">
                <div className="flex flex-row ml-3">
                    <div className="relative mr-5 mt-3 mb-3 group">
                      <img src={updatedUser?.avatarUrl} alt="user avatar" className="w-20 h-20 max-sm:mt-7 sm:w-50 sm:h-50 rounded-full object-cover"/>
                      <div className="w-20 h-20 max-sm:mt-7 sm:w-50 sm:h-50 absolute inset-0 bg-black/50 opacity-0 rounded-full cursor-pointer group-hover:opacity-100 transition-opacity flex items-center justify-center" onClick={() => {setAttribute("avatar"); setStateInputModal(true)}}>
                    ...
                    <div className="flex flex-col text-base mt-14 sm:text-2xl"> 
                        <h1>Email: {updatedUser?.email}</h1>
                        ...
                        <button className="w-40 h-10 my-5 text-white bg-green-500 box-border border border-transparent disabled:bg-gray-500 disabled:text-white/50 disabled:cursor-not-allowed hover:bg-green-700  shadow-xs font-medium leading-5 rounded-md text-sm px-4 mr-2 focus:outline-none cursor-pointer" disabled={change} onClick={() => {setAttribute("Confirm changes");setStateInputModal(true)}}>Confirm Changes</button>
                    </div>
                </div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-y-10 gap-x-20 mt-20 lg:px-18 xl:px-40 2xl:px-80">
              <div className="flex flex-col">
                <h1 className="text-white text-5xl mb-4">My Posts</h1>
                {posts.map(p => <PostCard key={p.id} updatedAt={p.updatedAt} title={p.title} description={p.description} id={p.id} editable={true}/>)}
              </div>
              <div className="flex flex-col lg:justify-self-end">
                <h1 className="text-white text-5xl mb-4">My Comments</h1>
                {comments.map(c => <CommentCard key={c.id} id={c.id} body={c.body} updatedAt={c.updatedAt} user={c.user} editable={true} />)}
              </div>
            </div>
        </div>
        ...
        {stateResultModal && user ? <ResultModal state={stateResultModal} result={result} setState={setStateResultModal} setResult={setResult} sucessText="updated data" failureText={failureText} /> : null}
    </div>
  )
}