...
export default function page() {
  ...
  const {register,reset,handleSubmit,watch,formState: { errors }} = useForm<IPost>()
  ...
  return (
    <div className='bg-blue-950 min-h-screen'>
        ...
        <div className="flex flex-col mt-3 ml-5 mr-5">
          ...
          <div className="my-3">
              <button type="button" className="inline-flex items-center text-white bg-blue-600 box-border border border-transparent hover:bg-blue-700  shadow-xs font-medium leading-5 rounded-md text-lg px-4 mr-2 w-35 h-10 focus:outline-none cursor-pointer" onClick={() => router.push(`/usersettings/${user?.id}`)}>
          ...
          <form className='flex flex-col text-white justify-center items-center' onSubmit={onSubmit}>
              <h1 className="text-white text-5xl my-3">Edit Post</h1>
              ...
              <button type="submit" className="w-40 h-10 mx-5 my-5 text-white bg-green-500 box-border border border-transparent hover:bg-green-700  shadow-xs font-medium leading-5 rounded-md text-xl px-4 mr-2 focus:outline-none cursor-pointer">Edit Post</button>
          </form>
          ...
          {state ? <ResultModal result={result} setResult={setResult} state={state} setState={setState} sucessText='Post updated' failureText={failureText} /> : null}
      </div>
    </div>
  )
}