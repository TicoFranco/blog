...
import { registerUser } from '@/app/services/Users/registerUser';
    ...
    <div className='bg-blue-950 min-h-screen'>
        ...
        <div className="flex flex-col justify-center items-center text-white mt-8">
            ...
            <form className='w-90 flex flex-col' onSubmit={onSubmit}>
                ...
                <div>
                   <input type="hidden" {...register("avatarUrl", {required: "Choose an avatar"})}/>
                   <h3 className='text-xl my-2.5 font-medium'>Choose an avatar</h3>
                   <div className='flex flex-row my-2'>
                     {avatars.map((avatar) => (<img key={avatar} src={avatar} onClick={() => setValue("avatarUrl",avatar,{ shouldValidate: true })} className={`w-16 h-16 rounded-full mr-2 cursor-pointer border-2 transition ${selectedAvatar === avatar ? "border-blue-600 scale-110" : "border-transparent"}`}/>))}
                   </div>
